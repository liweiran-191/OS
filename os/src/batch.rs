...
pub fn run_next_app() -> ! {
    let current = APP_MANAGER.inner.borrow().get_current_app();
    unsafe { APP_MANAGER.inner.borrow().load_app(current); }
    APP_MANAGER.inner.borrow_mut().move_to_next_app();
    unsafe extern "C" { fn __restore(_:usize); }
    unsafe {
